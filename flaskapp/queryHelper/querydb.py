...
def queryDb(db, parameters):
    ...
    extra_conditions = [(f" AND {condition['criteria']} = ?", condition['search_string']) for condition in parameters['conditions']
                        if condition['criteria'] != 'Text']
    ...
    for row in rows:
        ...
        for condition in parameters['conditions']:
            ...
            search_string = condition['search_string'].upper().replace(' ', '')
            ...
            for i in range(column_length - search_length + 1):
                ...
                if column_value[i] != search_string[0]:
                    continue
                ...
                similarity_ratio = difflib.SequenceMatcher(None, substring, search_string).ratio()
            ...
            if max_similarity < float(condition['similarity']):
                flag = 0
                break
        if flag:
